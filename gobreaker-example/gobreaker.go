...
//
// 熔断器还有限流的作用，就是过多的请求，类似于电流过大
// 这种流量限定我想起了tcp 中的拥塞控制算法，和熔断器的功能有些相似
...
func (cb *CircuitBreaker) Execute(req func() (interface{}, error)) (interface{}, error) {
	...
	// 相当于应该是对应同一个类型的任务进行操作，而不是不同任务类型
	result, err := req()
...
func (cb *CircuitBreaker) beforeRequest() (uint64, error) {
	...
	// 如果还用重试的机会就继续进行重试操作然后将on request 的数据加1
	cb.counts.onRequest()
